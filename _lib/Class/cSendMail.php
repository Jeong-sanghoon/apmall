...
/*
 =================================================================================
+ 안내 : 이 파일은 아래의 웹사이트 자료를 참고로 수정하였음을 알려드립니다.
...
+ 파일명 : cSendMail.php
+ 수정일 : 2015-07-27
+ 수정자 : mprd kim inho (eno@mprd.co.kr)
*/
class cSendMail {
	
	/* smtp 의 호스트 설정 : 아래는 gmail 일경우 */
	private $host="218.153.103.14";
	
	/* port */
	private $port="25";
	
	/* smtp 계정 아이디 입력 */
	private $smtp_id="webmaster";
	/* smtp 계정 비밀번호 입력 */
	private $smtp_pw="tlfzmdnpqaktmxj";
	...
	/*   smptp 서버에 접속을 한다. */
	function connect($host='', $port="") { //port 465
		if($this->debug) {
		...
		if(!$this->fp = fsockopen($host, $port, $errno, $errstr, 15)) {
			...
			echo "<BR/>SMTP(".$host.",".$port.") 서버접속에 실패했습니다.[".$errno.":".$errstr."]<BR/>";
			return false;
		}
	        	...
	        	if($this->host == "auto") {
	        		foreach($to as $email) {
	        			if($host = $this->get_mx_server($email)) {
	        				...
	        				if($conn) {
	        					$result = $this->smtp_send($email, $from, $data,$cc_mail,$bcc_mail);
	        					$this->close();
	        				}
	        			}
	        		}
	        		...
	        		foreach($to as $key=>$email){
	        			$conn = $this->connect($this->host);
	        			$result = $this->smtp_send($email, $from, $data,$cc_mail,$bcc_mail,$rel_to);
	        			$this->close();
	        		}
