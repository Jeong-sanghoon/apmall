<?
	...
	$qry = "
		...
		, SUM(CASE WHEN C.P_STATUS = 'P' AND DEL_YN = 'N' THEN C.P_QTY ELSE 0 END) AS STATUS_P
		, SUM(CASE WHEN C.P_STATUS = 'R' AND DEL_YN = 'N' THEN C.P_QTY ELSE 0 END) AS STATUS_R
		, SUM(CASE WHEN C.P_STATUS = 'E' AND DEL_YN = 'N' THEN C.P_QTY ELSE 0 END) AS STATUS_E
		FROM TB_ORDER A
...
<script type="text/javascript">
	...
	function doProc(){
		...
		if(confirm("선택한 항목의 상태를 변경하시겠습니까?")){
			...
			if(nChk > 0){alert("입고확인이 안된 상품이 있습니다"); return false;}
	...
	function doProcReturn(){
		...
		if(confirm("선택한 항목의 상태를 이전상태로 되돌립니다")){
			var arrChk = new Array();
			$('input[name="chkitem"]:checked').each(function(){
				arrChk.push($(this).val());
			});
			
			$('#pageaction').val('goods_ready');
			$('#arr_item_seq').val(arrChk);
			$('#status').val('C');
	...
	// 엑셀저장
	function doExcel(){
		location.href = "orderEnterReadyExcel.php?<?=$gStr?>";
	}
	...
	function doMakeBarcode(){
		var arr_orderid = new Array();
		var arr_itemid = new Array();
		
		$('input[name="chkitem"]:checked').each(function(){
			arr_orderid.push($(this).data('orderid'));
			arr_itemid.push($(this).data('itemid'));
		});
		...
		$.ajax({
			...
			success: function(obj){
				...
				for(var i = 0; i < arr_data.length; i++){
					...
					strHtml += '		<table border="0" cellspacing="0" cellpadding="0" class="test_tb_01 test_mt_10">';
					strHtml += '			<colgroup>';
					...
					strHtml += '				<td class="lf2">'+ arr_data[i].orderid +'</td>';
					strHtml += '			</tr>';
					strHtml += '			<tr>';
					strHtml += '				<th scope="row">아이템번호</th>';
					strHtml += '				<td class="lf2">'+ arr_data[i].itemid +'</td>';
					strHtml += '			</tr>';
					strHtml += '			<tr>';
					strHtml += '				<td colspan="2">';
					strHtml += '					<img src="<?=IMG_URL?>/barcode/'+ arr_data[i].barcodefile +'" />';
					strHtml += '				</td>';
					strHtml += '			</tr>';
					strHtml += '		</table>';
				}
	...
	// 입고스캐닝페이지(새창)
	function goScanBarcode(){
		window.open("_scanBarcode.php?type=enter");
	}
	
	// 구매정보팝업
	function popPurchase(item_seq, item_rowid){
		window.open('orderEnterReadyPopList.php?item_seq='+ item_seq +'&item_rowid='+ item_rowid, 'out_req', 'width=1280, height=800');
	}
</script>
	...
	<div id='contain'>
		...
		<div id='content_wrap'> <!-- 100%의 전체 컨텐츠 영역-->
			...
			<div class='content w100p'> <!-- 원하는 컨텐츠 넓이를 추가 w960 클래스는 width:960px란 의미 -->
				...
				<div class='mt10'>				
					<input type='button' value='입고명세서인쇄' class='btn_02 w110' onclick="javascript:popBarcode();" />
					<input type='button' value='액셀저장' class='btn_02 w80' onclick="javascript:doExcel();" />
					
					<p class='fr'>
						<input type='button' value='입고스캐닝' class='btn_02 w110' onclick="javascript:goScanBarcode();"/>
						<input type='button' value='상품준비되돌리기' class='btn_02 w110' onclick="javascript:doProcReturn();"/>
						<input type='button' value='입고완료' class='btn_02 w60' onclick="javascript:doProc();"/>
					</p>
				</div>
...
<?
	if($nTotalCnt > 0){
		foreach($rsList as $i=>$ds){
			...
			$ds['WEARRD_DT_STR'] = str_replace(' ', '<br>', $ds['WEARRD_DT']);
			
			$p_status = '입고대기';
			if($ds['STATUS_E'] > 0){
				if($ds['P_QTY'] == $ds['STATUS_E']) $p_status = '입고확인';
			}
?>
					<tr>
						<td><input name="chkitem" id="chkitem_<?=$ds['ITEM_SEQ']?>" type="checkbox" value="<?=$ds['ITEM_SEQ']?>" data-order_seq="<?=$ds['ORDER_SEQ']?>" data-orderid="<?=$ds['ORDERID']?>" data-itemid="<?=$ds['ITEMID']?>" data-barcode_url="<?=$ds['BARCODE_URL']?>" data-p_status="<?=$p_status?>" /></td>
						<td><?=$PageS?></td>
						...
						<td><?=$ds['REG_DT_STR']?></td>
						<td><?=$ds['WEARRD_DT_STR']?></td>
						<td class='bg_01'><a href="javascript:popPurchase('<?=$ds['ITEM_SEQ']?>', '<?=$ds['ITEM_ROWID']?>');"><?=$p_status?> (<?=number_format($ds['P_QTY'])?>)</a></td>
					</tr>
<?
		}
	}
...
?>
	...
	<!-- popup -->
	<div id="barcode_pop" class="dialog-popup" style="width:800px;">
		<div id='pop'>
			<div class='title'>
				<h2>입고명세서</h2>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" id="barcode_tb">
				<!-- ajax: doMakeBarcode()
				<tr>
					<td>
