...
<script type="text/javascript">
	...
	function doProcReturn(){
		...
		if(confirm("선택한 주문번호에 해당하는 상품이 모두 주문접수 상태로 돌아갑니다")){
			var arrChk = new Array();
			$('input[name="chkitem"]:checked').each(function(){
				arrChk.push($(this).data('order_seq'));
			});
			
			$('#pageaction').val('accept');
	...
	// 엑셀저장
	function doExcel(){
		location.href = "orderPayExcel.php?<?=$gStr?>";
	}
		...
		<div id='content_wrap'> <!-- 100%의 전체 컨텐츠 영역-->
			...
			<div class='content w100p'> <!-- 원하는 컨텐츠 넓이를 추가 w960 클래스는 width:960px란 의미 -->
				...
				<div class='mt10'>				
					...
					<p class='fr'>
						<input type='button' value='주문접수되돌리기' class='btn_02 w110' onclick="javascript:doProcReturn();"/>
						<input type='button' value='상품준비' class='btn_02 w60' onclick="javascript:doProc();"/>
					</p>
				</div>
...
?>
					<tr>
						<td><input name="chkitem" id="chkitem_<?=$ds['ITEM_SEQ']?>" type="checkbox" value="<?=$ds['ITEM_SEQ']?>" data-order_seq="<?=$ds['ORDER_SEQ']?>" /></td>
						<td><?=$PageS?></td>
