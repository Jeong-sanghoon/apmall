<?
	...
	foreach($rsList as $i=>$ds){
		...
		$ds['WEARSC_DT_STR'] = str_replace(' ', chr(10), $ds['WEARSC_DT']);
		...
		$sheet->setCellValue('H'.$row, $ds['WEARSC_DT_STR']);
	}
	
	//다운
	header('Content-Type: application/vnd.ms-excel');
	header('Content-Disposition: attachment;filename=order_enter_fin_'. date('YmdHis') .'.xlsx');
