		...
		$.ajax({
			...
			success: function(obj){
				if(obj.status == 0){
					...
					location.href = "orderEnterReadyPopList.php?<?=$gStr?>";
				}
			},
						...
						<tr>
							...
							<td class='lf'>
								<select name="p_status" id="p_status">
									<option value="R" <?=$cFnc->CodeString($ds['P_STATUS'], 'R', 'selected', '')?>>입고대기</option>
									<option value="E" <?=$cFnc->CodeString($ds['P_STATUS'], 'E', 'selected', '')?>>입고완료</option>
								</select>
							</td>
						</tr>
						-->
