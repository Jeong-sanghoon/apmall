<?
	...
	foreach($rsList as $i=>$ds){
		...
		$ds['GOODSRD_DT_STR'] = str_replace(' ', chr(10), $ds['GOODSRD_DT']);
		
		$sheet->setCellValue('A'.$row, $ds['DELIVERYID']);
		$sheet->setCellValue('B'.$row, $ds['GOODSRD_DT_STR']);
	...
	header('Content-Disposition: attachment;filename=order_out_req_'. date('YmdHis') .'.xlsx');
