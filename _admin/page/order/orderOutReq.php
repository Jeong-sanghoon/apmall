<?
	...
	$qry = "
		...
		, COUNT(B.`DELIVERY_ITEM_SEQ`) AS CNT, SUM(C.`SUMPRICE`) AS SUMPRICE, MAX(C.GOODSRD_DT) AS GOODSRD_DT
...
<script type="text/javascript">
	...
	// 엑셀저장
	function doExcel(){
		location.href = "orderOutReqExcel.php?<?=$gStr?>";
	}
	...
	function goScanBarcode(){
		window.open("_scanBarcode.php?type=out");
	}
</script>
	...
	<div id='contain'>
		...
		<div id='content_wrap'> <!-- 100%의 전체 컨텐츠 영역-->
			...
			<div class='content w100p'> <!-- 원하는 컨텐츠 넓이를 추가 w960 클래스는 width:960px란 의미 -->
				...
				<div class='mt10'>				
					<input type='button' value='출고명세서인쇄' class='btn_02 w110' onclick="javascript:popBarcode();" />
					<input type='button' value='액셀저장' class='btn_02 w80' onclick="javascript:doExcel();" />
					
					<p class='fr'>
						<input type='button' value='출고스캐닝' class='btn_02 w110' onclick="javascript:goScanBarcode();"/>
						<input type='button' value='입고완료되돌리기' class='btn_02 w110' onclick="javascript:doProcReturn();"/>
						<input type='button' value='출고완료' class='btn_02 w60' onclick="javascript:doProc();"/>
					</p>
				</div>
