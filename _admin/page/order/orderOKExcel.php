<?
	...
	$qry = "
		...
		, COUNT(B.`DELIVERY_ITEM_SEQ`) AS CNT, SUM(C.`SUMPRICE`) AS SUMPRICE, MAX(C.GOODSCF_DT) AS GOODSCF_DT
	...
	foreach($rsList as $i=>$ds){
		...
		$ds['GOODSCF_DT_STR'] = str_replace(' ', chr(10), $ds['GOODSCF_DT']);
		
		$sheet->setCellValue('A'.$row, $ds['DELIVERYID']);
		$sheet->setCellValue('B'.$row, $ds['GOODSCF_DT_STR']);
	...
	header('Content-Disposition: attachment;filename=order_OK_'. date('YmdHis') .'.xlsx');
