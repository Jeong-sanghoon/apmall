...
<script type="text/javascript">
	...
	function doProc(){
		...
		var gsWin = window.open('about:blank', 'out_proc', 'width=1280, height=800');
		$('#frmProc').attr('action', 'orderOutProcPop.php');
		$('#frmProc').attr('target', 'out_proc');
		$('#frmProc').submit();
	}
	...
	// 엑셀저장
	function doExcel(){
		location.href = "orderEnterFinExcel.php?<?=$gStr?>";
	}
		...
		<div id='content_wrap'> <!-- 100%의 전체 컨텐츠 영역-->
			...
			<div class='content w100p'> <!-- 원하는 컨텐츠 넓이를 추가 w960 클래스는 width:960px란 의미 -->
				...
				<div class='mt10'>				
					...
					<p class='fr'>
						<input type='button' value='입고대기되돌리기' class='btn_02 w110' onclick="javascript:doProcReturn();"/>
						<input type='button' value='출고요청' class='btn_02 w60' onclick="javascript:doProc();"/>
					</p>
				</div>
...
<?
	if($nTotalCnt > 0){
		foreach($rsList as $i=>$ds){
			...
			$ds['WEARSC_DT_STR'] = str_replace(' ', '<br>', $ds['WEARSC_DT']);
?>
					<tr>
						<td><input name="chkitem" id="chkitem_<?=$ds['ITEM_SEQ']?>" type="checkbox" value="<?=$ds['ITEM_SEQ']?>" data-order_seq="<?=$ds['ORDER_SEQ']?>" data-orderid="<?=$ds['ORDERID']?>" data-itemid="<?=$ds['ITEMID']?>" data-barcode_url="<?=$ds['BARCODE_URL']?>" /></td>
						<td><?=$PageS?></td>
...
?>
	...
	<!-- popup -->
	<div id="barcode_pop" class="dialog-popup" style="width:400px;">
		<div id='pop'>
			<div class='title'>
				<h2>입고명세서</h2>
			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class='tb_01_non mt10' id="barcode_tb">
				<colgroup>
					<col width='20%' />
					<col width='80%' />
				</colgroup>
