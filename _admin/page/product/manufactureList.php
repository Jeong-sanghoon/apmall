<?
	...
	if($manufacturename != ''){
		$qryWhere .= " AND MANUFACTURENAME like ?";
		array_push($arParam, '%'. $manufacturename .'%');		
	}
	
	$qry = "
		SELECT MANUFACTURE_SEQ, MANUFACTURENAME, REG_DT, MOD_DT, USERNAME, TEL, ADDR, USE_YN, EMAIL
...
<script type="text/javascript">
	...
	// 수정
	function goUpdate(manufacture_seq){
		location.href = "manufactureInput.php?manufacture_seq=" + manufacture_seq + "&<?=$gStr2?>";
	}

	// 입력
	function goInput(){
		location.href = "manufactureInput.php?<?=$gStr2?>";
	}
	...
	// 삭제
	function goDelete(manufacture_seq){
		if(confirm("선택한 게시물을 삭제하시겠습니까?")){
			$('#pageaction').val("DELETE");			
			$('#manufacture_seq').val(manufacture_seq);
	...
	// 상세
	function goDetail(manufacture_seq){
		location.href = "manufactureView.php?manufacture_seq="+ manufacture_seq +"&<?=$gStr2?>";
	}
	...
	<form method="post" name="frmProc" id="frmProc" action="">
		...
		<input type="hidden" name="manufacture_seq" id="manufacture_seq" value="">
			...
			<div class='content w100p'> <!-- 원하는 컨텐츠 넓이를 추가 w960 클래스는 width:960px란 의미 -->
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0" class='adtb_02 '>
					...
					<tr>
						...
						<td class='lf'>
							<input name="manufacturename" id="manufacturename" type="text" class='w200' value="<?=$manufacturename?>" onkeypress="javascript:doEnter();" />
						</td>
...
?>
					<tr>
						<td><input name="chkitem" id="chkitem_<?=$ds['MANUFACTURE_SEQ']?>" type="checkbox" value="<?=$ds['MANUFACTURE_SEQ']?>" /></td>
						<td><?=$PageS?></td>						
						<td><a href="javascript:goDetail('<?=$ds['MANUFACTURE_SEQ']?>');"><?=$ds['MANUFACTURENAME']?></a></td>
						<td><?=$ds['USERNAME']?></td>
						<td><?=$ds['EMAIL']?></td>
						<td><?=$ds['TEL']?></td>
						<td><?=$ds['ADDR']?></td>
						...
						<td>							
							<input type='button' value='수정' class='btn_02 w40' onclick="javascript:goUpdate('<?=$ds['MANUFACTURE_SEQ']?>');"/>
							<input type='button' value='삭제' class='btn_02 w40' onclick="javascript:goDelete('<?=$ds['MANUFACTURE_SEQ']?>');"/>
						</td>						
					</tr>
<?
		}
