...
<script type="text/javascript">
	...
	function goUpdate(adm_seq){
		location.href = "memberInput.php?user_seq=" + adm_seq + "&<?=$gStr2?>";
	}

	// 입력
	function goInput(){
		location.href = "memberInput.php?<?=$gStr2?>";
	}
	...
	// 상세
	function goDetail(user_seq){
		location.href = "memberView.php?user_seq="+ user_seq +"&<?=$gStr2?>";
	}
	...
	// 엑셀저장
	function doExcel(){
		location.href = "memberExcel.php?<?=$gStr?>";
	}
	...
	<form method="post" name="frmProc" id="frmProc" action="">
		...
		<input type="hidden" name="usre_seq" id="user_seq" value="">
...
?>
					<tr>
						<td><input name="chkitem" id="chkitem_<?=$ds['USER_SEQ']?>" type="checkbox" value="<?=$ds['USER_SEQ']?>" /></td>
						<td><?=$PageS?></td>
						<td class='bg_01'><a href="javascript:goDetail('<?=$ds['USER_SEQ']?>');"><?=$ds['USER_ID']?></a></td>
						<td><?=$ds['JOIN_TP']?></td>
						<td><?=$ds['USER_NM']?></td>
						<td><?=$ds['TEL']?></td>
						<td><?=$ds['REG_DT_STR']?></td>						
						...
						<td>							
							<input type='button' value='수정' class='btn_02 w40' onclick="javascript:goUpdate('<?=$ds['USER_SEQ']?>');"/>
							<input type='button' value='삭제' class='btn_02 w40' onclick="javascript:goDelete('<?=$ds['USER_SEQ']?>');"/>
						</td>						
					</tr>
<?
		}
