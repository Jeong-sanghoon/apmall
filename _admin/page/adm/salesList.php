<?
	...
	if($adm_nm != ''){
		$qryWhere .= " AND SALESNAME like ?";
		array_push($arParam, '%'. $salesname .'%');		
	}

	
	$qry = "
		SELECT SALES_SEQ, SALESNAME, PER, REG_DT, MOD_DT, USE_YN 
...
<script type="text/javascript">
	...
	// 수정
	function goUpdate(sales_seq){
		location.href = "salesInput.php?sales_seq=" + sales_seq + "&<?=$gStr2?>";
	}

	// 입력
	function goInput(){
		location.href = "salesInput.php?<?=$gStr2?>";
	}
	...
	// 삭제
	function goDelete(sales_seq){
		if(confirm("선택한 게시물을 삭제하시겠습니까?")){
			$('#pageaction').val("DELETE");			
			$('#sales_seq').val(sales_seq);
	...
	// 상세
	function goDetail(sales_seq){
		location.href = "salesView.php?sales_seq="+ sales_seq +"&<?=$gStr2?>";
	}
			...
			<div class='content w100p'> <!-- 원하는 컨텐츠 넓이를 추가 w960 클래스는 width:960px란 의미 -->
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0" class='adtb_02 '>
					...
					<tr>
						...
						<td class='lf'>
							<input name="salesname" id="salesname" type="text" class='w200' value="<?=$salesname?>" onkeypress="javascript:doEnter();" />
						</td>
...
?>
					<tr>
						<td><input name="chkitem" id="chkitem_<?=$ds['SALES_SEQ']?>" type="checkbox" value="<?=$ds['SALES_SEQ']?>" /></td>
						<td><?=$PageS?></td>
						<td class='bg_01'><a href="javascript:goDetail('<?=$ds['SALES_SEQ']?>');"><?=$ds['SALESNAME']?></a></td>
						<td><?=$ds['PER']?></td>
						<td><?=$ds['REG_DT_STR']?></td>
						<td><?=CODE_USE_YN($ds['USE_YN'])?></td>
						<td>							
							<input type='button' value='수정' class='btn_02 w40' onclick="javascript:goUpdate('<?=$ds['SALES_SEQ']?>');"/>
							<input type='button' value='삭제' class='btn_02 w40' onclick="javascript:goDelete('<?=$ds['SALES_SEQ']?>');"/>
						</td>						
					</tr>
<?
		}
