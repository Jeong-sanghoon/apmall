<?
	...
	if($storage_nm != ''){
		$qryWhere .= " AND STORAGE_NM like ?";
		array_push($arParam, '%'. $storage_nm .'%');		
	}

	if($manager_nm != ''){
		$qryWhere .= " AND MANAGER_NM like ?";
		array_push($arParam, '%'. $manager_nm .'%');		
	}
	
	
	$qry = "
		SELECT STORAGE_SEQ, CATEGORY, CODE_SEQ, STORAGE_NM, ADDR, MANAGER_NM, TEL
		, REG_DT, MOD_DT, USE_YN, POST_NO, ADDR_DETAIL, EMAIL 
...
<script type="text/javascript">
	...
	// 수정
	function goUpdate(storage_seq){
		location.href = "storageInput.php?storage_seq=" + storage_seq + "&<?=$gStr2?>";
	}

	// 입력
	function goInput(){
		location.href = "storageInput.php?<?=$gStr2?>";
	}
	...
	// 삭제
	function goDelete(storage_seq){
		if(confirm("선택한 게시물을 삭제하시겠습니까?")){
			$('#pageaction').val("DELETE");			
			$('#storage_seq').val(storage_seq);
	...
	// 상세
	function goDetail(storage_seq){
		location.href = "storageView.php?storage_seq="+ storage_seq +"&<?=$gStr2?>";
	}
	...
	<form method="post" name="frmProc" id="frmProc" action="">
		...
		<input type="hidden" name="storage_seq" id="storage_seq" value="">
			...
			<div class='content w100p'> <!-- 원하는 컨텐츠 넓이를 추가 w960 클래스는 width:960px란 의미 -->
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0" class='adtb_02 '>
					...
					<tr>
						<th scope="row">창고명</th>
						<td class='lf'>
							<input name="storage_nm" id="storage_nm" type="text" class='w200' value="<?=$storage_nm?>" onkeypress="javascript:doEnter();" />
						</td>						
						<th scope="row">담당자</th>
						<td class='lf'>
							<input name="manager_nm" id="manager_nm" type="text" class='w200' value="<?=$manager_nm?>" onkeypress="javascript:doEnter();" />
						</td>						
					</tr>
				</table>			
				</form>
...
?>
					<tr>
						<td><input name="chkitem" id="chkitem_<?=$ds['STORAGE_SEQ']?>" type="checkbox" value="<?=$ds['STORAGE_SEQ']?>" /></td>
						<td><?=$PageS?></td>
						<td class='bg_01'><a href="javascript:goDetail('<?=$ds['STORAGE_SEQ']?>');"><?=$ds['CATEGORY']?></a></td>
						<td><?=$ds['CODE_SEQ']?></td>
						<td><?=$ds['STORAGE_NM']?></td>
						<td><?=$addr_str?></td>
						<td><?=$ds['MANAGER_NM']?></td>
						...
						<td><?=$ds['REG_DT_STR']?></td>
						<td>							
							<input type='button' value='수정' class='btn_02 w40' onclick="javascript:goUpdate('<?=$ds['STORAGE_SEQ']?>');"/>
							<input type='button' value='삭제' class='btn_02 w40' onclick="javascript:goDelete('<?=$ds['STORAGE_SEQ']?>');"/>
						</td>						
					</tr>
<?
		}
