<?
	...
	$dToday = date('Y-m-d');	
	$d1w 	= date('Y-m-d', strtotime('-3 month');
			...
			<div class='content w100p'> <!-- 원하는 컨텐츠 넓이를 추가 w960 클래스는 width:960px란 의미 -->
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0" class='adtb_02 '>
					...
					<tr>
						...
						<td class='lf'>
							<input name="search_text" id="search_text" type="text" class='w200' value="<?=$search_text?>" onkeypress="javascript:doEnter();" />
						</td>
						<th scope="row">아이템코드</th>
						<td class='lf'>
							<input name="search_text" id="search_text" type="text" class='w200' value="<?=$search_text?>" onkeypress="javascript:doEnter();" />
						</td>
					</tr>
