<?
	...
	if($country_nm != ''){
		$qryWhere .= " AND COUNTRY_NM like ?";
		array_push($arParam, '%'. $country_nm .'%');		
	}
	
	
	$qry = "
		SELECT RATE_SEQ, KRW, FCR, REG_DT, COUNTRY_NM, ADM_SEQ 
...
<script type="text/javascript">
	...
	// 수정
	function goUpdate(rate_seq){
		location.href = "rateInput.php?rate_seq=" + rate_seq + "&<?=$gStr2?>";
	}

	// 입력
	function goInput(){
		location.href = "rateInput.php?<?=$gStr2?>";
	}
	

	// 삭제
	function goDelete(rate_seq){
		if(confirm("선택한 게시물을 삭제하시겠습니까?")){
			$('#pageaction').val("DELETE");			
			$('#rate_seq').val(rate_seq);
	...
	function goDetail(system_cd){
		location.href = "rateView.php?rate_seq="+ system_cd +"&<?=$gStr2?>";
	}
	...
	<form method="post" name="frmProc" id="frmProc" action="">
		...
		<input type="hidden" name="rate_seq" id="rate_seq" value="">
			...
			<div class='content w100p'> <!-- 원하는 컨텐츠 넓이를 추가 w960 클래스는 width:960px란 의미 -->
				...
				<table border="0" width="100%" cellspacing="0" cellpadding="0" class='adtb_02 '>
					...
					<tr>
						...
						<td class='lf'>
							<input name="country_nm" id="country_nm" type="text" class='w200' value="<?=$country_nm?>" onkeypress="javascript:doEnter();" />
						</td>						
					</tr>
				</table>			
				</form>
...
?>
					<tr>
						<td><input name="chkitem" id="chkitem_<?=$ds['RATE_SEQ']?>" type="checkbox" value="<?=$ds['RATE_SEQ']?>" /></td>
						<td><?=$PageS?></td>
						<td class='bg_01'><a href="javascript:goDetail('<?=$ds['RATE_SEQ']?>');"><?=$ds['COUNTRY_NM']?></a></td>
						<td><?=$ds['KRW']?></td>
						<td><?=$ds['FCR']?></td>
						<td><?=$ds['REG_DT_STR']?></td>						
						<td>							
							<input type='button' value='수정' class='btn_02 w40' onclick="javascript:goUpdate('<?=$ds['RATE_SEQ']?>');"/>
							<input type='button' value='삭제' class='btn_02 w40' onclick="javascript:goDelete('<?=$ds['RATE_SEQ']?>');"/>
						</td>						
					</tr>
<?
		}
