<?
	...
	$_NAVITITLE = "RequestOrder > OrderRequest";	
	...
	if($usermstid != ''){
		...
		$qry = "
			SELECT A.*, B.*, B.ROWID AS USERITEMID, B.PRODUCTNAME AS ITEM_NM, A.PRICE AS PRICESUM
			...
			<div class='content w100p'> <!-- 원하는 컨텐츠 넓이를 추가 w960 클래스는 width:960px란 의미 -->
				...
				<div class='w100p'><!-- 컨텐츠 가로길이 줄임-->
					...
					<div class='mt20'>
						<h2>1. Order Info</h2>
						<p class="fr">
							<input type='button' value='MemberSearch' class='btn_02 w100' onclick="javascript:popUser();"/>
						</p>
					</div>
					...
					<table border="0" width="100%" cellspacing="0" cellpadding="0" class='adtb_02 mt10'>
						...
						<tr>
							<th scope="row">DepositAmount<br/>(auto)</th>
							<td class='lf'><input name="price_f" id="price_f" type="text" class='w95p' value="<?=$rs[0]['PRICE_F']?>" readonly></td>
							<th scope="row">DepositFee<br/>(auto)</th>
							<td class='lf'>
								<input type="text" name="depositfeesum" id="depositfeesum" class="w30p" value="<?=$rs[0]['DEPOSITFEE']?>">
								DepositRate : <input type="text" name="depositper" id="depositper" class="w30p" value="<?=$per?>" onkeyup="javascript:calSumArea();">
								<input type='button' value='reset' class='btn_b01 w80' onclick="javascript:doDepositInit();"/>
							</td>
						</tr>
						<tr>
							<th scope="row">CODAmount</th>
							<td class='lf'><input name="price_b" id="price_b" type="text" class='w95p' value="<?=$rs[0]['PRICE_B']?>" readonly></td>
							<th scope="row">OrderIDRequest</th>
							<td class='lf'>
...
?>
					...
					<table border="0" width="100%" cellspacing="0" cellpadding="0" class='adtb_02 mt10'>
						...
						<tr>
							<th scope="row">Destnation Address</th>
							<td class='lf'>
					...
					<div class='mt20'>
						<h2 class="fl">3. Item Info</h2>
						<p class="fr">
							<input type='button' value='SearchItem' class='btn_02 w100' onclick="javascript:popSearch();"/>
						</p>
<?
	if($usermstid == ''){
?>
						<p class="fr mr10">
							<input type="button" value="AddItem" class='btn_02 w50' onclick="javascript:addItem('', '', '', '');">
							<input type="button" value="ItemCopy" class='btn_02 w80' onclick="javascript:addCopyItem();">
						</p>
<?
	}
?>
					...
					<table border="0" width="100%" cellspacing="0" cellpadding="0" class='adtb_02 mt10' id="tb_3">
						...
						<tr>
							...
							<th scope="row">InvoiceName</th>
							<th scope="row">OptionField</th>
							<th scope="row">OptionValue</th>
							<th scope="row">Qty</th>
							<th scope="row">Price(Unit Price)</th>
							<th scope="row">SumPrice</th>
							<th scope="row">DepositFee</th>
...
?>
					...
					<div class='center mt20'>
						<input type='button' value='TempSave' class='btn_b01 w80' onclick="javascript:doTempSave();"/>
						<input type='button' value='Input' class='btn_b01 w80' onclick="javascript:doProc();"/>
<?
	if($usermstid != ''){
?>
						<input type='button' value='Cancel' class='btn_b02 w80' onclick="javascript:doCancel();"/>
<?
	}
?>
					</div>
